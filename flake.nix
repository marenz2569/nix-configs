{
  ...
  let
    ...
    packages.marenz-frickelkiste-nixos-rebuild = pkgs.writeScriptBin "marenz-frickelkiste-nixos-rebuild" ''
      nixos-rebuild --flake ${self}?submodules=1#marenz-frickelkiste -L $@
    '';
  in
  {
    package.x86_64-linux.default = self.nixosConfiguration.marenz-frickelkiste.config.system.build.vm;
